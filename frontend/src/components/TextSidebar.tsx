...
import { uploadTextFiles, fetchTextDocs, deleteTextDoc } from '../api/text';

export const TextSidebar: React.FC = () => {
  const { docs, currentIndex, setCurrentIndex, setDocs, removeDoc, isLoading } = useTextStore();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await uploadTextFiles(files, currentProject?.id);
      if (result.total_uploaded > 0) {
        const newDocs = await fetchTextDocs(currentProject?.id);
        setDocs(newDocs);
        setUploadMessage(`Uploaded ${result.total_uploaded} document(s)`);
      }
      if (result.total_failed > 0) {
        const names = result.failed.map((f) => f.filename ?? 'unknown').join(', ');
        setUploadMessage((prev) =>
          prev ? `${prev}. Failed: ${names}` : `Failed: ${names}`
        );
      }
  ...
  const handleRemoveDoc = async (e: React.MouseEvent, filename: string) => {
    e.stopPropagation();
    try {
      await deleteTextDoc(filename);
      removeDoc(filename);
    } catch (err) {
