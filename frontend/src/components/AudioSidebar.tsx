...
import { uploadAudioFiles, fetchAudioFiles, deleteAudioFile } from '../api/audio';
import { formatDurationMs } from '../utils/time';

export const AudioSidebar: React.FC = () => {
  const { files, currentIndex, setCurrentIndex, setFiles, removeFile, isLoading } = useAudioStore();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await uploadAudioFiles(uploadFiles, currentProject?.id);
      if (result.total_uploaded > 0) {
        const newFiles = await fetchAudioFiles(currentProject?.id);
        setFiles(newFiles);
      ...
      <div className="w-48 bg-th-bg-sidebar border-r border-th-border flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-th-text-secondary text-center text-sm">
            <p>No audio files</p>
            <p className="mt-2 text-xs">Upload .mp3, .wav, .ogg, .flac, or .m4a files</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-48 bg-th-bg-sidebar border-r border-th-border flex flex-col">
      ...
      <div className="p-2 border-b border-th-border bg-th-bg-footer">
        <h2 className="text-sm font-semibold text-th-text-primary">Audio Files ({files.length})</h2>
